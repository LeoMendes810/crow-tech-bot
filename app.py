...
>>> [{datetime.now().strftime('%H:%M:%S')}] Escaneando mercado em tempo real.
        """, language="bash")
        
        c_b1, c_b2 = st.columns([4, 1])
        with c_b1: st.button("🚀 INICIAR ALGORITMO", use_container_width=True)
        with c_b2: 
    ...
    with tab2:
        ...
        ca, cb = st.columns(2)
        with ca:
            st.markdown('<p class="config-label">Gatilho RSI (Entrada)</p>', unsafe_allow_html=True)
            st.markdown('<p class="instruction-text">Define o nível de força para compra. O robô entrará quando o RSI estiver abaixo deste valor.</p>', unsafe_allow_html=True)
            ...
            st.markdown('<p class="instruction-text">Nível de segurança para encerrar a posição e evitar perdas maiores.</p>', unsafe_allow_html=True)
        ...
        with cb:
            ...
            st.markdown('<p class="instruction-text">Valor de lucro total desejado para o dia. Ao atingir, o bot encerra a atividade.</p>', unsafe_allow_html=True)
            ...
            st.markdown('<br><p class="config-label">Média Móvel de Referência (EMA)</p>', unsafe_allow_html=True)
            st.markdown('<p class="instruction-text">Média exponencial utilizada pelo algoritmo para confirmar a direção do preço.</p>', unsafe_allow_html=True)
            st.text_input("EMA", value="EMA 20 (CONFIGURAÇÃO NATIVA)", disabled=True, label_visibility="collapsed")
        st.markdown('</div>', unsafe_allow_html=True)

    with tab3:
        st.markdown('<div class="product-card">', unsafe_allow_html=True)
        st.markdown("### 🔐 Credenciais de API Corretora")
        st.info("Insira suas chaves para vincular o Portal Elite à sua carteira. Suas chaves são processadas em ambiente isolado.")
        st.text_input("API KEY (BINANCE/OUTRAS)", type="password")
